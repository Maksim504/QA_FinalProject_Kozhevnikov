...
> По умолчанию UI-тесты запускаются в **headless**-режиме (без окна браузера).
> Чтобы запустить их с **видимым окном**, задайте переменную окружения:
> ```powershell
> $env:HEADLESS = "0"      # Windows PowerShell
> export HEADLESS=0        # Linux/Mac (bash)
> ```
> После этого обычная команда `pytest -m ui -v` откроет окно Chrome.
> В новом терминале снова действует headless-режим (переменная живёт
> только в текущей сессии); принудительно вернуть его можно через
> `HEADLESS=1`. На работу в CI/CD переключатель не влияет.
